...
;    sac a, #-7, w3          ; preserve current i value
;    ;;;;;;;;;;; Proportional ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;    mov [w1], w5            ; fetch kp to w5
;    mac w4*w5, a            ; multiply current in with kp and accumulate
;    ;;;;;;;;;;; Derivative ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;    mov [w1+20], w2         ; fetch prevIn value to w2
;    mov [w1+4], w5          ; fetch kd to w5
;    sub w4, w2, w4          ; subtract prevIn from current in and result load to w4
;    mac w4*w5, a            ; multiply difference (in-prevIn) with kd and accumulate
;    ;;;;;;;;;;; Output limit ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;    sac a, #-7, w0          ; load PID result to w0
;    mov [w1+10], w4         ; fetch max value
;    sub w4, w0, w2          ; subtract max - out to make comparison
;    bra ge, OUT_LT_MAX      ; if out <= max jump to OUT_LT_MAX
;    mov w4, w0              ; alter w0 with max
;    bra OUT_GT_MAX
;OUT_LT_MAX:
;    mov [w1+8], w4          ; fetch min value
;    sub w4, w0, w2          ; subtract max - out to make comparison
;    bra le, OUT_IN_RANGE    ; if out >= min jump to OUT_IN_RANGE
;    mov w4, w0              ; alter w0 with min
;    bra OUT_LT_MIN
;OUT_IN_RANGE:
;    mov w3, [w1+12]         ; store current i value
;OUT_GT_MAX:
;OUT_LT_MIN:
;  return
; .end
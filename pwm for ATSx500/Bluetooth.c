...
static unsigned int PeriodStartFlag;
static unsigned int WidthStartFlag;
static unsigned int ConnectedFlag;
static unsigned int PingFlag;
static unsigned int UpDownFlag;
static signed int Ocrs;
...
{	
	...
	RxByte = U1RXREG;
	if(PeriodStartFlag == 1)								// enters here only when RxByte contains second byte of received frame with Length property
	{
		...
		if(RxFrameBufferIndex == 4)
		{
			RxFrameBufferIndex = 0;
			T2CONbits.TON = 0;			// stop timer 2
			PR2 = (RxFrameBuffer[0]-48)*1000+(RxFrameBuffer[1]-48)*100+(RxFrameBuffer[2]-48)*10+(RxFrameBuffer[3]-48);
			T2CONbits.TON = 1;			// start timer 2
			PeriodStartFlag = 0;
			U1TXREG = 'O';
			U1TXREG = 'K';
			U1TXREG = '\n';
		}								// prevent entering to this block of code when reads rest of frame
	}
	else if(WidthStartFlag == 1)								// enters here only when RxByte contains second byte of received frame with Length property
	{
		//Length = (unsigned int)RxByte;					// retein Length property used to iterate through the RxFrameBuffer
		RxFrameBuffer[RxFrameBufferIndex++] = RxByte;	// write it also to first byte of the RxFrameBuffer (not used enywhere, but coppied just in case)
		if(RxFrameBufferIndex == 4)
		{
			RxFrameBufferIndex = 0;
			OC1RS = (RxFrameBuffer[0]-48)*1000+(RxFrameBuffer[1]-48)*100+(RxFrameBuffer[2]-48)*10+(RxFrameBuffer[3]-48);
			WidthStartFlag = 0;
			U1TXREG = 'O';
			U1TXREG = 'K';
			U1TXREG = '\n';
	...
	else if(RxByte == 'p')								// enters here only at the begining of a frame
	{
		PeriodStartFlag = 1;								// prepares for next byte redout being Length property
	}
	else if(RxByte == 'w')								// enters here only at the begining of a frame
	{
		WidthStartFlag = 1;								// prepares for next byte redout being Length property
	}	
	else if(RxByte == 'u')								// enters here only at the begining of a frame
	{
	...
	else if(RxByte == '1')								// enters here only at the begining of a frame
	{
		Ocrs = OC1RS;
		if(UpDownFlag)
		{
			Ocrs++;
			if(Ocrs > PR2) Ocrs = PR2;
		}
		else
		{
			Ocrs--;
	...
	else if(RxByte == '2')								// enters here only at the begining of a frame
	{
		Ocrs = OC1RS;
		if(UpDownFlag)
		{
	...
	else if(RxByte == '3')								// enters here only at the begining of a frame
	{
		Ocrs = OC1RS;
		if(UpDownFlag)
		{
	...
	else if(RxByte == '4')								// enters here only at the begining of a frame
	{
		Ocrs = OC1RS;
		if(UpDownFlag)
		{
		...
		OC1RS = Ocrs;
	}								// enters here when Lenght property is known to the end of the frame
...
///////// Transmiter Setup /////////////////////////////////////////////////////
		...
		IEC0bits.U1TXIE = 0;		// Transmiter Interrupt Disable
